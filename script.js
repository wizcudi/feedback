...
(()=>{
    ...
    const submitHandler = event => {
        ...
        //send feedback item to server -- POST fetch call
...
(() => {
    const clickHandler = (event) => {
        ...
        if (upvoteIntention){
            ...
            // disable upvite button (prevent double-click, spam)
            upvoteBtnEl.disabled = true;
    
            //select the upvite count element within upvote btn
            const upvoteCountEl = upvoteBtnEl.querySelector('.upvote__count')
    
            // get current displayed upvote count, as number (+)
    ...
    fetch(`${BASE_API_URL}/feedbacks`) //GET fetch call
...
(() => {
    const clickHandler = event => {
        ...
        // stop fucntion if click happens outside btn in list
        if(clickedEl.className === 'hashtags'){
            return; // return is how you stop a function
        }
        ...
        feedbackListEl.childNodes.forEach(childNode => {
            // stop iteration if its a text node
            ...
            //remove all feedback item from list if company names are not equal
